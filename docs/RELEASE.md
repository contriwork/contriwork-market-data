...
- Registry trust policies in place:
  ...
  - **NuGet**: Active trust policy scoped to `Repository: contriwork-market-data` on the `ContriWork` account. The account name is **case-sensitive** on trust-policy lookup.
...
```markdown
[Unreleased]: https://github.com/contriwork/contriwork-market-data/compare/vX.Y.Z...HEAD
[X.Y.Z]: https://github.com/contriwork/contriwork-market-data/compare/v<PREV>...vX.Y.Z
```
...
unzip -p /tmp/rel/Contriwork.MarketData.X.Y.Z.nupkg README.md | head -1
# expect: "# Contriwork.MarketData (.NET)"
...
tar -xOzf dist/contriwork_market_data-X.Y.Z.tar.gz contriwork_market_data-X.Y.Z/README.md | head -1
# expect: "# contriwork-market-data (Python)"
```
...
```bash
curl -s "https://api.nuget.org/v3-flatcontainer/contriwork.marketdata/index.json"
```
...
```bash
curl -sL "https://api.nuget.org/v3-flatcontainer/contriwork.marketdata/X.Y.Z/contriwork.marketdata.X.Y.Z.nupkg" -o /tmp/pkg.nupkg
...
```bash
curl -sL "https://registry.npmjs.org/@contriwork/market-data/-/market-data-X.Y.Z.tgz" -o /tmp/npm.tgz
